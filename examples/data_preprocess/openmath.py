...
if __name__ == '__main__':
    ...
    train_1M_dataset = train_1M_dataset.map(function=make_map_fn('train_1M'), with_indices=True)
    train_5M_dataset = datasets.Dataset.from_list([x for x in train_5M_dataset if x is not None])
    train_1M_dataset = datasets.Dataset.from_list([x for x in train_1M_dataset if x is not None])

    local_dir = args.local_dir
    hdfs_dir = args.hdfs_dir

    train_5M_dataset.to_parquet(os.path.join(local_dir, 'train_5M.parquet'))
    train_1M_dataset.to_parquet(os.path.join(local_dir, 'train_1M.parquet'))
